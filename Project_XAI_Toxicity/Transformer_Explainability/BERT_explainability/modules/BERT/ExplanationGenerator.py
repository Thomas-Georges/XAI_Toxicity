...
class Generator:
    ...
    def generate_rollout(self, input_ids, attention_mask, start_layer=0, index=None):
        # This returns a tuple where (typically) output.attentions is a list of attention matrices.
        output_attentions = self.model(input_ids=input_ids, attention_mask=attention_mask)[1]
        
        # Each attention matrix is of shape (batch, num_heads, seq_length, seq_length).
        all_layer_attentions = []
        for attn in output_attentions:
            # Average over the heads: compute (sum over heads / num_heads)
            avg_heads = (attn.sum(dim=1) / attn.shape[1]).detach()
            all_layer_attentions.append(avg_heads)
        
        # Compute the rollout by aggregating the attention matrices.
        rollout = compute_rollout_attention(all_layer_attentions, start_layer=start_layer)
        
        # Optionally, adjust the rollout matrix.
        rollout[:, 0, 0] = 0
        return rollout
        #return rollout[:, 0]
